...
public class Select{
    ...
    public <T> int selectIndex(T[] items, Comparator<T> comp, int kthLowest, int size){
        if(size < 1){
            throw new AluRuntimeException("cannot select from empty array (size < 1)");
        }else if(kthLowest > size){
            throw new AluRuntimeException("Kth rank is larger than size. k: " + kthLowest + ", size: " + size);
        }
        int idx;
