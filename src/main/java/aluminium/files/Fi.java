package aluminium.files;

import aluminium.*;
import aluminium.Files.*;
import aluminium.util.*;
import aluminium.util.io.*;
 ...
 * <p>
 * Because some of the file types are backed by composite aluminium.files and may be compressed (for example, if they are in an Android .apk
...
public class Fi{
    ...
    public static Fi tempFile(String prefix){
        try{
            ...
            throw new AluRuntimeException("Unable to create temp file.", ex);
        }
    }
    ...
    private static void copyFile(Fi source, Fi dest){
        try{
            ...
            throw new AluRuntimeException("Error copying source file: " + source.file + " (" + source.type + ")\n" //
     ...
     */
    public File file(){
        if(type == FileType.external) return new File(Aluminium.files.getExternalStoragePath(), file.getPath());
        return file;
    }
    ...
    public InputStream read(){
        ...
        try{
            ...
            if(file().isDirectory())
                throw new AluRuntimeException("Cannot open a stream to a directory: " + file + " (" + type + ")", ex);
            throw new AluRuntimeException("Error reading file: " + file + " (" + type + ")", ex);
        }
    }
    ...
    public Reader reader(String charset){
        ...
        try{
            ...
            throw new AluRuntimeException("Error reading file: " + this, ex);
        }
    }
    ...
    public String readString(String charset){
        ...
        try{
            ...
            throw new AluRuntimeException("Error reading layout file: " + this, ex);
        }finally{
    ...
    /**
     * Attempts to memory map this file in READ_ONLY mode. Android aluminium.files must not be compressed.
     * @throws AluRuntimeException if this file handle represents a directory, doesn't exist, or could not be read, or memory mapping fails, or is a {@link FileType#classpath} file.
     */
    ...
    /**
     * Attempts to memory map this file. Android aluminium.files must not be compressed.
     * @throws AluRuntimeException if this file handle represents a directory, doesn't exist, or could not be read, or memory mapping fails, or is a {@link FileType#classpath} file.
     */
    public ByteBuffer map(FileChannel.MapMode mode){
        if(type == FileType.classpath) throw new AluRuntimeException("Cannot map a classpath file: " + this);
        ...
        try{
            ...
            throw new AluRuntimeException("Error memory mapping file: " + this + " (" + type + ")", ex);
        }finally{
            Streams.close(raf);
        }
    }
     ...
     * @throws AluRuntimeException if this file handle represents a directory, if it is a {@link FileType#classpath} or
     * {@link FileType#internal} file, or if it could not be written.
     */
    public OutputStream write(boolean append){
        if(type == FileType.classpath) throw new AluRuntimeException("Cannot write to a classpath file: " + file);
        if(type == FileType.internal) throw new AluRuntimeException("Cannot write to an internal file: " + file);
        parent().mkdirs();
        try{
            ...
            if(file().isDirectory())
                throw new AluRuntimeException("Cannot open a stream to a directory: " + file + " (" + type + ")", ex);
            throw new AluRuntimeException("Error writing file: " + file + " (" + type + ")", ex);
        }
    }
    ...
    public void write(InputStream input, boolean append){
        ...
        try{
            ...
            throw new AluRuntimeException("Error stream writing to file: " + file + " (" + type + ")", ex);
        }finally{
            Streams.close(input);
    ...
    /** Recursively iterates through all aluminium.files in this directory.
    ...
    /** Recursively iterates through all aluminium.files in this directory and adds them to an array.
     * Directories are not handled. */
    ...
    /** Recursively iterates through all aluminium.files in this directory and adds them to a newly allocated array.*/
     ...
     * array.
     * @throws AluRuntimeException if this file is an {@link FileType#classpath} file.
     */
    public Fi[] list(){
        if(type == FileType.classpath) throw new AluRuntimeException("Cannot list a classpath directory: " + file);
     ...
     * @param filter the {@link FileFilter} to filter aluminium.files
     * @throws AluRuntimeException if this file is an {@link FileType#classpath} file.
     */
     ...
     * handle represents a file and not a directory. On the desktop, an {@link FileType#internal} handle to a directory on the
     * classpath will return a zero length array.
     * @param filter the {@link FilenameFilter} to filter aluminium.files
     * @throws AluRuntimeException if this file is an {@link FileType#classpath} file.
     */
    ...
    /**
     * Returns true if this file is a directory. Always returns false for classpath aluminium.files. On Android, an
     ...
     * @throws AluRuntimeException if this file is the root.
     */
    ...
    public boolean mkdirs(){
        if(type == FileType.classpath) throw new AluRuntimeException("Cannot mkdirs with a classpath file: " + file);
        if(type == FileType.internal) throw new AluRuntimeException("Cannot mkdirs with an internal file: " + file);
        return file().mkdirs();
    }

    /**
     * Returns true if the file exists. On Android, a {@link FileType#classpath} or {@link FileType#internal} handle to a
     * directory will always return false. Note that this can be very slow for internal aluminium.files on Android!
     */
    public boolean exists(){
        switch(type){
            case internal:
    ...
    public boolean delete(){
        if(type == FileType.classpath) throw new AluRuntimeException("Cannot delete a classpath file: " + file);
        if(type == FileType.internal) throw new AluRuntimeException("Cannot delete an internal file: " + file);
        return file().delete();
    }
     ...
     * a directory, this directory is copied into it recursively, overwriting existing aluminium.files, or 3) if the destination doesn't
     * exist, {@link #mkdirs()} is called on the destination and this directory is copied into it recursively.
     * @throws AluRuntimeException if the destination file handle is a {@link FileType#classpath} or {@link FileType#internal}
    ...
    public void copyTo(Fi dest){
        ...
        if(dest.exists()){
            if(!dest.isDirectory()) throw new AluRuntimeException("Destination exists but is not a directory: " + dest);
        }else{
            dest.mkdirs();
            if(!dest.isDirectory()) throw new AluRuntimeException("Destination directory cannot be created: " + dest);
        }
     ...
     * @throws AluRuntimeException if this or {@param dest} is not a valid directory, or copying fails.
     * */
    public void copyFilesTo(Fi dest){
        if(!isDirectory()) throw new AluRuntimeException("Source folder must be a directory: " + this);
        if(dest.exists() && !dest.isDirectory()) throw new AluRuntimeException("Destination folder must be a directory: " + dest);
     ...
     * @throws AluRuntimeException if the source or destination file handle is a {@link FileType#classpath} or
     * {@link FileType#internal} file.
     */
    public void moveTo(Fi dest){
        switch(type){
            case classpath:
                throw new AluRuntimeException("Cannot move a classpath file: " + file);
            case internal:
                throw new AluRuntimeException("Cannot move an internal file: " + file);
            case absolute:
            case external:
     ...
     * for {@link FileType#classpath} aluminium.files. On Android, zero is returned for {@link FileType#internal} aluminium.files. On the desktop, zero
     * is returned for {@link FileType#internal} aluminium.files on the classpath.
     */
